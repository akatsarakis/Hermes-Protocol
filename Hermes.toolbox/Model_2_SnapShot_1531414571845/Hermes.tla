...
killANode == 
    /\aliveNodes' = aliveNodes \ {CHOOSE k \in NODES: \A m \in NODES: k <= m}
    /\ UNCHANGED <<msgs, nodeState, nodeTS, nodeLastWriter, issuedWriteTS, receivedAcks>>
-------------------------------------------------------------------------------------
HRead(n) ==  
    /\ nodeState[n] = "valid"
...
HSendVals(n) == 
    ...
    /\ send([type        |-> "VAL", 
             version     |-> nodeTS[n].version, 
             tieBreaker  |-> nodeTS[n].tieBreaker])
    /\ UNCHANGED <<nodeTS, nodeLastWriter, issuedWriteTS,
...
HNext == 
    \/ \E n \in NODES:       
                     \/ HFollowerActions(n)
                     \/ HCoordinatorActions(n)
    \/ killANode \* this is for failures
=============================================================================
\* Modification History
\* Last modified Thu Jul 12 17:11:11 BST 2018 by akatsarakis
